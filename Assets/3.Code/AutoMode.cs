...
public class AutoMode : MonoBehaviour
{
    ...
    private List<string> _myWordList = new List<string>();
    ...
    private IEnumerator AutoCoroutine()
    {
        while (isAuto)
        {
            ...
            {
                //preWord = _myWordList[Random.Range(0, _myWordList.Count)];

                KeyValuePair<string, int> firstWord = _myWordDict.First();
                _seoTest.wordText.text = firstWord.Key;
                _myWordDict[firstWord.Key] = 1;

                //_myWordDict[preWord] = 1;
            ...
            else
            {
                ...
                string findWord = _myWordList.Find(word => word.LastOrDefault().Equals(lastLetter));
                print($"다음 단어: {findWord}");
                _seoTest.wordText.text = findWord;

                _myWordDict[findWord] = 1;
            }
